...
namespace Projet.CirSim
{
    class Tv
    {
        ...
        public Tv()
        {
            ...
            if (Circuit.Horloge != null) s = s + Circuit.compoattaches.Count;
        ...
        {
            EvaluCas(l);
            foreach (Composant element in Circuit.compoattaches)
        ...
        public void SetSortie_seq()
        {
            for (int i = 0; i < ep2; i++)
            {
                EvaluerCas_seq(i);
                foreach (Composant c in Circuit.lesComposants) { c.evalue = false; }
                foreach (Composant c in Circuit.compoattaches) { c.evalue = false; }
            }
        }
